...
# Row-level fields that are PRESENTATION/decorative, not data content. These
# carry visual asset references (e.g. an image slug filename) that are optional
# polish — the renderer degrades gracefully when they are empty. They must NOT
# be counted by the extraction quality gate (a missing image is not a missing
# fact), and they are intentionally allowed to be blank.
TEMPLATE_PRESENTATION_FIELDS: dict[str, set[str]] = {
    "sort_card": {"image"},
    # vs_card.py treats Emoji/Notes/Category as optional/auto-filled
    # (renderer fillna("")'s them); Weight defaults to 1.0 so it's never
...
class TemplateDataset(BaseModel):
    ...
    image_sourcing_guide: dict[str, str] = Field(
        default_factory=dict,
        description=(
            "Operator-facing map of image slug filename -> one-line human visual "
            "description. Presentation metadata only; routed to data_manifest.json "
            "and never consumed by the renderer. Optional; defaults to empty."
        ),
    )
